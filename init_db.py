...
task = Task('title1', 'detail', status='DONE', price=1, estimate=4, team=Team.query.get(1), start_time=miami.utils.get_last_monday().replace(hour=10))
...
task = Task('title2', 'detail2', status='DONE', price=2, estimate=4, team=Team.query.get(1), start_time=miami.utils.get_last_monday().replace(hour=12))
ts = TimeSlot(miami.utils.get_last_monday().replace(hour=12), 3600, User.query.get(1), partner=User.query.get(2))
task.time_slots.append(ts)
db.session.add(task)
db.session.commit()

task = Task('title3', 'detail3', status='DONE', price=2, estimate=4, team=Team.query.get(1), start_time=miami.utils.get_last_monday().replace(hour=14))
ts = TimeSlot(miami.utils.get_last_monday().replace(hour=14), 3600, User.query.get(2), partner=User.query.get(1))
task.time_slots.append(ts)
db.session.add(task)
db.session.commit()

task = Task('title4', 'detail4', status='READY', price=2, estimate=4, team=Team.query.get(1), start_time=miami.utils.get_last_monday().replace(hour=15))
ts = TimeSlot(miami.utils.get_last_monday().replace(hour=15), 3600, User.query.get(2), partner=User.query.get(1))
task.time_slots.append(ts)
db.session.add(task)
db.session.commit()

task = Task('title6', 'detail4', status='READY', price=1, estimate=4, team=Team.query.get(1), start_time=miami.utils.get_last_monday().replace(hour=15))
db.session.add(task)
db.session.commit()

task = Task('title5', 'detail5', status='DONE', price=2, estimate=4, team=Team.query.get(1), start_time=(miami.utils.get_last_monday() - timedelta(days=5)).replace(hour=14))
ts = TimeSlot((miami.utils.get_last_monday() - timedelta(days=5)).replace(hour=14), 3600, User.query.get(2))
task.time_slots.append(ts)
db.session.add(task)
db.session.commit()

task = Task('title7', 'detail5', status='DONE', price=2, estimate=4, team=Team.query.get(1), start_time=miami.utils.yestoday().replace(hour=14))
